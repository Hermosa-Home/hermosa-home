const CACHE='hermosa-v4';
const ASSETS=['./index.html','./gestionale.html','./manifest.json','./manifest-gestionale.json'];
...
self.addEventListener('fetch',function(e){e.respondWith(caches.match(e.request).then(function(c){return c||fetch(e.request).catch(function(){return c;});}));});
